<nav class="navbar fixed-top navbar-expand-md navbar-dark d-print-none">
    <div class="container">
                ...
                <ul class="list-group list-group-flush">
                  @foreach (Auth::user()->unreadNotifications as $notification)
                    <li class="list-group-item">
                      <a href="{{ url('author/transactions/'.$notification->data['transaction_id']) }}" class="text-sm">
                        A client want's to {{ $notification->data['amount'] ?? ''}} USD
                        </a>
                      </li>
                  @endforeach
                </ul>
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <div class="d-flex flex-column flex-md-row flex-fill align-items-stretch align-items-md-center">
          <ul class="navbar-nav">
            ...
            @if (auth()->user()->type == 1)
              <li class="nav-item">
                <a class="nav-link" href="{{ route('author.chats') }}">
                  <span class="nav-link-title">
            ...
            @if (auth()->user()->type == 1)
              <li class="nav-item">
                <a class="nav-link" href="{{ route('author.verifications') }}" >
                  <span class="nav-link-title">
                    Verifier
                  </span>
                </a>
            ...
            @if (auth()->user()->type == 1)
              <li class="nav-item">
                <a class="nav-link" href="{{ route('author.commission') }}" >
                  <span class="nav-link-title">
                    Commisions
                  </span>
                </a>
            ...
            @if (auth()->user()->type == 1)
              <li class="nav-item">
                <a class="nav-link" href="{{ route('author.competitor') }}" >
                  <span class="nav-link-title">
                    Competitor Stats
                  </span>
                </a>
            ...
            <li class="nav-item dropdown">
              ...
              <div class="dropdown-menu">
                ...
                @if (auth()->user()->type == 1  || auth()->user()->type == 2 )
                  <a class="dropdown-item" href="{{ route('author.how-to-vids') }}" >
                    <span class="nav-link-title">
                      How to Videos
                    </span>
                  </a>
